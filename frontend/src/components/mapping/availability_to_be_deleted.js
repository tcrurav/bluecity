...
export class Availability extends React.Component {
  constructor(props) {
    ...
    this.checkGeolocationAvailability = this.checkGeolocationAvailability.bind(
      this
    );
  ...
  componentDidMount() {
    this.findAllBoxesInAParking(this.props.location.state.parking.id).then(
      (newState) => {
    ...
    if (!this.socket) {
      ...
      this.socket.on("refresh", (data) => {
        if (
          ...
          data.parking_changed === this.props.location.state.parking.id
        )
          this.refresh();
  ...
  findOutGreenRedOrOrange(data) {
    ...
    if (
      (!data.occupied && reservationExpired) ||
      (this.props.location.state.checkingForRenting &&
        !data.occupied &&
        reservationExpired)
    ) {
      return FREE;
      ...
      .then((res) => {
        this.findAllBoxesInAParking(this.props.location.state.parking.id).then(
          (newState) => {
            ...
            this.activateCountdown();
            this.checkOpenBoxPossible();
            this.socket.emit("something-changed", {
              who_changed_it: apiUser.id,
              parking_changed: this.props.location.state.parking.id,
            });
          }
        );
      })
    ...
    this.reservationInterval = setInterval(() => {
      ...
      const reservation_time_left =
        ...
        (new Date().getTime() -
          new Date(
            this.state.boxes[
              this.state.boxReservedByThisUser
            ].lastReservationDate
          ).getTime());
        ...
        <MyContainer>
          <Row>
            <Card className="m-2">
              ...
              <Card.Img
                variant="top"
                src={`${process.env.REACT_APP_BASEURL}/parking${this.props.location.state.parking.id}.jpg`}
              />
              <Card.Body>
                <Card.Title>
                  {this.state.boxes.length} boxes in total
                </Card.Title>
                {/* <Card.Text> */}
                <Row className="pt-2">
                  <Col>
                    ...
                    {this.state.free} available
                    <br />
                    <OrangeMarker>
                      <FontAwesomeIcon icon={faMapMarkerAlt} />
                    </OrangeMarker>{" "}
                    {this.state.reserved} reserved
                    <br />
                  </Col>
                  <Col>
                    <Row>
                      {this.state.boxes.map((b, index) => {
                        switch (this.findOutGreenRedOrOrange(b)) {
                          case OCCUPIED:
                            return <MyRedCol key={b.id}>{index + 1}</MyRedCol>;
                          case FREE:
                            return (
                              <MyGreenCol
                                key={b.id}
                          ...
                          case RESERVED:
                            return (
                              <MyOrangeCol key={b.id}>{index + 1}</MyOrangeCol>
                            );
                          default:
                            console.log("This case should never take place");
                            return <></>;
                    ...
                    <Row className="mt-1">
                      <Col>
                        {this.state.openBoxPossible ? (
                          ...
                          >
                            Open box nº{this.state.boxReservedByThisUser + 1}{" "}
                            &gt;&gt;
                          </Button>
                        ) : (
                          <></>
            ...
            <Col>
              ...
              THIS_USER_HAS_NO_RESERVATION ? (
                <>
                  ...
                  Click on the Box number to reserve a parking box.
                </>
              ) : (
                <>
                  <BlueInfo>
                  ...
                  Your reservation for box nº
                  {this.state.boxReservedByThisUser + 1} in parking{" "}
                  {this.props.location.state.parking.name} will expire in{" "}
                  {this.formatTimeLeft()}
                </>
              )}
            </Col>
            ...
            <Col>
              {this.state.geolocationAvailable ? (
                <>
                  ...
                  Geolocation activated. This parking is{" "}
                  {this.state.distanceToParking
                    ? this.state.distanceToParking.toFixed(2)
                    : ""}
                ...
                <>
                  ...
                  Geolocation not available. Please activate it.
                </>
              )}
            </Col>
          </Row>
